...
vector<int> dp((int)1e5 + 5, (int)1e9);
...
int main()
{
    ...
    // dp[0]=0;
    // // ith state ka answer nikal chuke ho
    // for(int i=0;i<=n-2;i++)
    // {
    ...
    // dp[i] min cost to go from (i....n-1)
    ...
    // dp[i] means ith state ka answer nikal chuka hai
    // and with help of that we are cal ans for dp[i-1],dp[i-2]
